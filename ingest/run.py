...
IMPORT VENDORS
  ...
  OS / Distro
  nvd           NVD CVE data via Limozacloud mirror, incremental by _state.csv sha256
                (requires: sync nvd; swap source with NVD_MIRROR_REPO)
...
def main():
    ...
    if command == "truncate":
        ...
        if not [a for a in args[1:] if not a.startswith("--")]:
            manifests = [
                Path(DIRS["nvd"])    / "_state.imported.csv",
