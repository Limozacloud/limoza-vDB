...
The shared weakness definition (name, mitigations, consequences, likelihood, …) is **not**
duplicated per record — it lives once in the `cwe` dictionary table, pre-populated by
`import cwe` from the [CWE source](../datasources/cwe.md) and keyed by `id`.
